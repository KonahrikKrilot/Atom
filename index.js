...
client.on('message', message =>{
    if(message.content === "tu fais quoi?"){
        message.channel.sendMessage('Je moccupe du serv! :heart:');
        console.log('répond à tfq');
    }
})

client.on('message', message =>{
    if(message.content === "Merci Atom"){
        message.channel.sendMessage('Derien! :heart:');
        console.log('répond à tfq');
    }
})

client.on('message', message =>{
    if(message.content === "tu est vivant?"){
        message.channel.sendMessage('bah oui je suis vivant ! :heart:');
        console.log('répond à tu est vivant?');
    }
})

client.on('guildMemberAdd', member =>{
    let embed = new Discord.RichEmbed()
        .setDescription(':tada: **' + member.user.username + '** a rejoint la ' + member.guild.name)
        .setFooter('Nous sommes désormais ' + member.guild.memberCount)
    member.guild.channels.get('562737969727930373').send(embed)
})

client.on('guildMemberRemove', member =>{
    let embed = new Discord.RichEmbed()
        .setDescription(':cry: **' + member.user.username + '** a quitté la ' + member.guild.name)
        .setFooter('Nous sommes désormais ' + member.guild.memberCount)
    member.guild.channels.get('562720721822810119').send(embed)
...
client.on("message", message => {
    ...
    if (args[0].toLowerCase() === prefix + "clear") {
        ...
        message.channel.bulkDelete(parseInt(count) + 1); return message.channel.send("Vous avez supprimer le nombres de message demander");
    }
